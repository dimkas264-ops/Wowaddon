...
-- 3.3.5: LibDeflate может отсутствовать, проверяем
local LibDeflate
if LibStub then
    local success, lib = pcall(LibStub, "LibDeflate")
    if success then LibDeflate = lib end
end

-- Если LibDeflate нет — создаём заглушку
if not LibDeflate then
    LibDeflate = {
        CompressDeflate = function(self, data) return data end,
        DecompressDeflate = function(self, data) return data end,
        CompressZlib = function(self, data) return data end,
        DecompressZlib = function(self, data) return data end,
    }
end
...
-- ============================================================
-- APPLIES CHECK (фильтрация по классу/фракции/расе)
-- ============================================================
            ...
            for str in string.gmatch(text, "[^/]+") do
                ...
                for entry in string.gmatch(str, "!?[%w%d]+") do
                    ...
                    v = (not(gendercheck or uppercase == class or entry == race or
                        entry == faction or playerLevel >= level or uppercase == addon.game or entry == customClass or entry == locale or customCheck) == state)
...
function addon.AddGuide(guide)
    ...
    if loadedGuide and addon.settings.profile.debug then
        ...
        elseif guide.version > loadedGuide.version then
            debugText = fmt('Newer guide for (%s) already exists (%s) >= checkGuide (%d)',
                guide.key, guide.version, loadedGuide.version)
        else
            debugText = fmt('Loading new version for (%s) v%d', guide.key, guide.version)
        end
    ...
    if loadedGuide then
        ...
        if not guide.lowPrio and guide.defaultFor and not addon.defaultGuide then
            addon.defaultGuide = guide
        end
    end
    return true
end
...
-- ============================================================
-- DATA INTEGRITY (упрощено для 3.3.5)
-- ============================================================

local function CheckDataIntegrity(str, h1, mode)
    if h1 then
        if mode == 58 then
            -- 3.3.5: упрощённая проверка без Battle.net привязки
            local S = {}
            local i, j
            local buffer = {}
...
function addon.CacheGuide(key, guide, enabledFor, guideVersion, metadata)
    ...
    if type(guide) == "table" then
        ...
        addon.db.profile.guides[profileKey] = addon.BuildCacheObject(guide, enabledFor, guideVersion, metadata, key)
    end
end
...
function addon.ImportGuide(guide, text, defaultFor, cache)
    if addon.db then
        ...
        if errorMsg ~= "#0" and importedGuide and
            ...
            addon.CacheGuide(importedGuide.key, guide, importedGuide.enabledFor, importedGuide.version, metadata)
        end
...
function addon.RegisterGuide(groupOrContent, text, defaultFor)
    ...
    elseif addon.addonLoaded then
        local importedGuide, errorMsg = addon.ParseGuide(groupOrContent, text, defaultFor)
...
function addon.A32(tbl)
    ...
    while i <= length - 15 do
        local x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16 =
            readfunc(tbl, i + offset, i + 15 + offset)
...
-- ============================================================
-- READ CACHE DATA (упрощено для 3.3.5 — без Battle.net)
-- ============================================================

local cachedData = {}
function addon.ReadCacheData(mode)
    if not cachedData.base then
        -- 3.3.5: Упрощённая генерация ключа без Battle.net
        local base = "RXP335"
        base = base:lower()
        local k = #base
        if k > 16 then
        ...
        for i = 0, 15 do
            buffer[bitand(-i, 0xf)] = bitxor(bitxor(
                bitxor(buffer[bitand(15 - i, 0xf)],
                    buffer[bitand(13 - i, 0xf)]),
...
function addon.ImportString(str, workerFrame, showFrame)
    ...
    if tonumber(base) < addon.version then
        addon.settings:UpdateImportStatusHistory(
            L"Incompatible guide game %d version vs %d", tonumber(base), addon.version)
    ...
    for hash, mode, content in str:gmatch("(%-?%d+)(%D)([A-Za-z0-9%+%/=]+)%%") do
        local validData, dataOrError = CheckDataIntegrity(content, tonumber(hash), strbyte(mode))
...
function addon.LoadEmbeddedGuides(maxIterations)
    ...
    for n = #embeddedGuides, 1, -1 do
        ...
        if maxIterations and iterations > maxIterations then break end

        local guideData = embeddedGuides[n]
        embeddedGuides[n] = nil

        if guideData.cache then
            addon.ImportGuide(guideData.groupOrContent, guideData.text, guideData.defaultFor, true)
        else
            ...
            elseif not guideData.text then
                local length = guideData.groupOrContent:len()
                ...
                if header then
                    ...
                    for line in header:gmatch("[^\r\n]+") do
                        if subgroup and name and group and enabledFor and enabled then break end
                        line = line:gsub("%--.*$", "")
                        line = line:gsub("^%s*(#.+)%s*<<%s*(.+)", function(l, t)
                            if not applies(t) then return "" else return l end
                        end)
                        if not enabled then
            ...
            elseif enabled then
                guide, errorMsg, metadata = addon.ParseGuide(guideData.groupOrContent, guideData.text, guideData.defaultFor, true, group, key)
                enabled = not errorMsg
                if key and metadata then
                    local cleanup = {}
                    for guideKey, data in pairs(RXPCData.guideMetaData) do
                        if data.key == guide.key then tinsert(cleanup, guideKey) end
                    end
        ...
        addon.comms.PrettyPrint("Error trying to load a guide already parsed: " .. group .. "/" .. name)
                end
...
function addon.BuildGuideKey(arg1, arg2, arg3)
    if type(arg1) == "table" then
        return fmt("%s|%s|%s", arg1.group, arg1.subgroup or '', arg1.name)
    elseif arg1 and arg3 then
        return fmt("%s|%s|%s", arg1, arg2 or '', arg3)
    end
end
...
function addon.LoadCachedGuides()
    ...
    for key, guideData in pairs(addon.db.profile.guides) do
        ...
        if enabled then
            if guideData.metadata and guideData.metadata.length == addon.ReadCacheData("string") then
                ...
                if guide:find("^--" .. addon.ReadCacheData("string")) then
                    ...
                    if metadata then guideData.metadata = metadata end
                else
                    guide = nil
                end
            end
            ...
            if not errorMsg and guide then
                ...
                addon.comms.PrettyPrint(L('Unable to decode cached guide (%s), removed'), key)
...
function addon:FetchGuide(guide)
    if not guide then return end
    if guide.steps then return guide end

    local cache = addon.guideCache[guide.key]
    if cache then
        local success, result = pcall(cache, guide)
        if success and result then
            for k, v in pairs(result) do
                if k ~= "parse" then guide[k] = v end
            end
...
local function parseLine(linetext, step, parsingLogic)
    if not parsingLogic then parsingLogic = addon.functions end
    addon.step = step
...
-- В конец файла GuideLoader.lua добавьте:

-- ============================================================
-- FILE GUIDE REGISTRATION (для гайдов из папки Guides\)
-- ============================================================

function addon:RegisterFileGuide(guideData)
    if not guideData or not guideData.group or not guideData.name then
        print("|cff33ff99RXP|r: |cffff0000Error: Invalid guide data|r")
        return false
    end
    
    -- Конвертируем таблицу в строку формата RXP
    local guideString = self:FormatGuideToString(guideData)
    
    -- Парсим и добавляем
    local guide, errorMsg = self.ParseGuide(guideString, nil, guideData.defaultFor, true, guideData.group)
    
    if guide and not errorMsg then
        self.AddGuide(guide)
        print("|cff33ff99RXP|r: Loaded guide: |cff00ff00" .. (guide.displayname or guide.name) .. "|r")
        return true
    else
        print("|cff33ff99RXP|r: |cffff0000Failed to load guide: " .. (guideData.name or "Unknown") .. "|r")
        if errorMsg then print("  Error: " .. errorMsg) end
        return false
    end
end

-- Форматирование таблицы в строку RXP
function addon:FormatGuideToString(data)
    local lines = {}
    
    table.insert(lines, "#group " .. data.group)
    if data.subgroup then
        table.insert(lines, "#subgroup " .. data.subgroup)
    end
    table.insert(lines, "#name " .. data.name)
    if data.displayname then
        table.insert(lines, "#displayname " .. data.displayname)
    end
    if data.defaultFor then
        table.insert(lines, "#defaultfor " .. data.defaultFor)
    end
    if data.next then
        table.insert(lines, "#next " .. data.next)
    end
    if data.wotlk then
        table.insert(lines, "#wotlk")
    end
    
    for _, step in ipairs(data.steps or {}) do
        table.insert(lines, "step")
        if step.sticky then
            table.insert(lines, "#sticky")
        end
        for _, element in ipairs(step.elements or {}) do
            -- Упрощённая конвертация элементов
            local line = ""
            if element.tag == "goto" then
                line = string.format(".goto %s,%.1f,%.1f", element.zone or "", element.x or 0, element.y or 0)
            elseif element.tag == "accept" then
                line = string.format(".accept %d", element.questId or 0)
            elseif element.tag == "turnin" then
                line = string.format(".turnin %d", element.questId or 0)
            elseif element.tag == "complete" then
                line = string.format(".complete %d", element.questId or 0)
            elseif element.tag == "text" or element.tag == "note" then
                line = element.text or ""
            end
            if element.text and element.text ~= "" and element.tag ~= "text" and element.tag ~= "note" then
